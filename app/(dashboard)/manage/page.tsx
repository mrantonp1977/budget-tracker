...
import { Cherry, Plus, Trash, TreeDeciduous, TrendingDown, TrendingUp } from 'lucide-react';
...
import CreateProductCategoryDialog from '../_components/CreateProductCategoryDialog';
import DeleteProductCategoryDialog from '../_components/DeleteProductCategoryDialog';
    ...
    <>
      <div className="bg-card mt-8">
        <div className="flex flex-wrap items-center justify-between px-8 py-8">
          <div className="">
            <p className="text-3xl font-bold dark:text-cyan-500">Manage Settings</p>
      ...
      <div className="flex flex-col px-4 p-4">
        ...
        <ProductCategoryList type="chestnuts" />
        <ProductCategoryList type="cherries" />
...
function ProductCategoryList({ type }: { type: ProductType }) {
  ...
  const dataAvailable = productCategoriesQuery.data && productCategoriesQuery.data.length > 0;

  return (
    <SkeletonWrapper isLoading={productCategoriesQuery.isLoading}>
      <Card className="mt-4">
        <CardHeader>
          <CardTitle className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              {type === 'cherries' ? (
                <Cherry className="h-12 w-12 items-center rounded-lg bg-purple-400/10 text-purple-500" />
              ) : (
                <TreeDeciduous className="h-12 w-12 items-center rounded-lg bg-purple-400/10 text-green-500" />
              )}
              <div className="p-5 dark:text-amber-500">
                {type === 'chestnuts'
                  ? 'Chestnuts Categories'
                  : 'Cherries Categories'}
                <div className="text-sm text-muted-foreground py-2">
                  Sorted by name
                </div>
            ...
            <CreateProductCategoryDialog
              type={type}
              successCallback={() => productCategoriesQuery.refetch()}
              trigger={
                <Button className="gap-2 text-sm" variant={'secondary'}>
                  <Plus className="h-4 w-4" />
                  Create Category
        ...
        {dataAvailable && (
          <div className="grid grid-flow-row gap-2 p-2 sm:grid-flow-row sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {productCategoriesQuery.data.map((productCategory: ProductCategory) => (
              <ProductCategoryCard key={productCategory.name} productCategory={productCategory} />
            ))}
          </div>
        )}
      </Card>
...
function ProductCategoryCard({ productCategory }: { productCategory: ProductCategory }) {
  return (
    <div className="flex border-separate flex-col justify-between rounded-md border shadow-md shadow-black/[0.1] dark:shadow-white/[0.1]">
      <div className="flex flex-col items-center gap-2 p-4">
        <span className="text-3xl" role="img">
        ...
        <span className="">{productCategory.name}</span>
      </div>
